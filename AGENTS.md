...
- `RunComplianceChecks` wraps each plugin's `RunChecks()` in `defer recover()` — panicked plugins are logged via `*slog.Logger` and retained in results with zero findings (not skipped). Callers pass `slog.Default()`
