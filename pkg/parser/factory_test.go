...
func TestFactory_ValidOPNsense(t *testing.T) {
	...
	device, _, err := parser.NewParserFactory().CreateDevice(
		context.Background(),
...
func TestFactory_UnknownRootElement(t *testing.T) {
	...
	_, _, err := parser.NewParserFactory().CreateDevice(
		context.Background(),
...
func TestFactory_ContextCancelled_BlockingReader(t *testing.T) {
	...
	go func() {
		_, _, err := parser.NewParserFactory().CreateDevice(ctx, pr, "", false)
		done <- err
	}()
