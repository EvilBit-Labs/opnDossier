...
| CWE-312 | Cleartext storage of sensitive data | The `sanitize` command redacts credentials via field-pattern matching. Device-specific field names (e.g., pfSense `<bcrypt-hash>`) require explicit patterns in `internal/sanitizer/rules.go`. | Mitigated |
