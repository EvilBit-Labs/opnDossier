...
The audit engine scans a configurable directory for `.so` files and loads any plugin that exports `var Plugin compliance.Plugin`. See [Security Model](#security-model) below for the trust model and preflight hardening behavior.
...
### Trust Model and Threat Posture

Dynamic plugins execute with full process privileges. There is no sandboxing, signature verification, or privilege separation. When you load a `.so` file via `--plugin-dir`, that file runs inside the opnDossier process with the same user, filesystem, and network permissions as the audit command itself.

A stderr warning is emitted whenever `--plugin-dir` is supplied:
...
**The trade-offs by design:**

- **Full process privileges.** Plugins run in-process with identical user/filesystem/network permissions. No sandbox or capability restriction is applied.
- **No signature verification.** The loader does not verify checksums, signatures, or provenance. Any `.so` file in the plugin directory is loaded.
- **Opt-in only.** Loading requires explicit `--plugin-dir` or the presence of a `./plugins` directory. Plugins are never fetched remotely.

### Preflight Hardening Checks

Before `plugin.Open()` is invoked, the loader performs preflight hardening checks on each `.so` file (added in PR #587, Phase A). These checks reject common footguns and emit a structured audit log per load attempt:

- **Symlink rejection.** The loader uses `os.Lstat` to detect symlinks and rejects them outright. `plugin.Open()` follows symlinks, so an attacker with write access to the plugin directory could otherwise redirect a `.so` path to any file on the filesystem.

- **Group/world-writable file rejection.** On POSIX systems, plugin files with group-writable (`0o020`) or world-writable (`0o002`) permission bits are rejected. This closes CWE-732 scenarios where another local account could swap the file between audits.

- **Group/world-writable directory rejection.** The containing directory is also checked for group/world-writable bits. File permissions alone are insufficient — a writable parent directory allows an attacker to unlink and replace the plugin.

- **Absolute path requirement.** The `--plugin-dir` path must be absolute. Relative paths are rejected so audit logs unambiguously identify the artifact.

- **SHA-256 digest logging.** Before loading, the loader computes and logs the SHA-256 of each plugin file (capped at 64 MiB to prevent memory exhaustion). The digest appears in the structured audit log, enabling post-audit forensics.

**Audit logging:** Every load attempt produces exactly one structured log entry — INFO for accepted loads, WARN for rejections — with fields `plugin`, `path`, `sha256`, `mode`, `owner_uid`, `mtime`, `size_bytes`, `verdict`, and `reason`. Operators can filter by `verdict` to track load failures without losing any attempts.

**Windows behavior:** POSIX permission bits are meaningless on NTFS, so the group/world-writable checks are skipped at runtime when `runtime.GOOS == "windows"`. Symlink and absolute-path checks still apply. The `owner_uid` field is logged as `unavailable` on Windows.

**Phase B follow-ups** (post-v1.5): Planned hardening includes owner-UID checks (refuse `.so` files not owned by the process UID or a configured allowlist), configurable size cap (`--plugin-max-size-mb`), path denylist (`/tmp`, `/var/tmp`, `/dev/shm`, `$HOME` when EUID==0), filename allowlist (no NUL / shell metacharacters / path separators), optional `plugins.sha256` manifest enforcement, documented seccomp/landlock sandboxing recipe, and out-of-process plugin isolation (à la HashiCorp go-plugin).

### Securing Plugin Directories

To minimize risk when using `--plugin-dir`:

- **Restrict filesystem permissions.** The plugin directory and all `.so` files should be owned by a trusted user and readable only by that user. Avoid world-writable paths, `/tmp`, and shared CI scratch directories.
- **Review plugin source code.** Build plugins from known-good checkouts. Treat every `.so` the same way you would treat an unsigned executable.
- **Pin build toolchain versions.** Plugins must be built with the same Go version and module dependencies as the main opnDossier binary. Version mismatches are caught at load time, but only after the `.so` is mapped into the process.
- **Use absolute paths.** Always pass a fully-qualified `--plugin-dir` to avoid ambiguity in audit logs.
- **Monitor audit logs.** Check for preflight rejections (`verdict=rejected`) in structured logs to detect permission or symlink issues before they escalate.

### Cross-References

- **GOTCHAS §2.5 — Dynamic Plugin Trust Model:** Comprehensive discussion of the trust model, Phase A hardening, and Phase B roadmap.
- **docs/user-guide/commands/audit.md § Dynamic Plugin Security:** User-facing guidance on securing plugin directories.
