...
- **Known limitation (follow-up):** `pkg/model.WebGUI` has no `Port` field, so a non-default web-configurator port is assumed to be 443/80. Closing this needs a schema + converter change (tracked separately), not a red-mode-only fix.
- **ExploitNotes safety:** red `ExploitNotes` carry impact/context only (never weaponized/step-by-step guidance). Enforced by `FindInstructionalContent` (`exploit_notes_safety.go`) run over the actual generated notes plus a golden-file gate — not authoring discipline. The `--audit-blackhat` sharper-tone variant exists at the template layer but is not yet wired to a CLI flag.
