...
// TestGolden_HTMLConsistency ensures multiple HTML renders produce identical output.
func TestGolden_HTMLConsistency(t *testing.T) {
	result := buildMultiSectionResult()
	...
	for i := range 5 {
		...
		outputs[i] = string(normalizeOutput(buf.Bytes()))
	}

	for i := 1; i < len(outputs); i++ {
		require.Equal(t, outputs[0], outputs[i],
			"HTML output from run %d should match run 1", i+1)
	}
}

// TestGolden_HTMLStructure verifies essential HTML structure elements are present.
func TestGolden_HTMLStructure(t *testing.T) {
	testCases := goldenDiffTestCases()
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			require.Contains(t, output, "<html lang=\"en\">")
			require.Contains(t, output, "</html>")
			require.Contains(t, output, "<style>")
			require.Contains(t, output, "Generated by opnDossier")

			// Must NOT contain external resource references
			require.NotContains(t, output, "href=\"http", "should not reference external CSS")
			require.NotContains(t, output, "src=\"http", "should not reference external JS")
			require.NotContains(t, output, "cdn", "should not reference CDN")
		})
	}
}
