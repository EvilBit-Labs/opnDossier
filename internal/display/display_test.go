...
func TestTerminalDisplayShouldShowNavigationHints(t *testing.T) {
	td := NewTerminalDisplayWithOptions(Options{Theme: LightTheme()})
...
func TestThemeProperties(t *testing.T) {
	tests := []struct {
		...
		{"Light theme properties", LightTheme(), true, "background"},
		{"Dark theme properties", DarkTheme(), true, "foreground"},
		{"Custom theme properties", CustomTheme(), false, "nonexistent"},
	}
