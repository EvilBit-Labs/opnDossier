...
type EnrichedOpnSenseDocument struct {
	*schema.OpnSenseDocument
	...
	NATSummary *schema.NATSummary `json:"natSummary,omitempty"`
}
...
func EnrichDocument(cfg *schema.OpnSenseDocument) *EnrichedOpnSenseDocument {
	if cfg == nil {
		return nil
	}
...
//
// Returns nil if the input configuration is nil.
func generateStatistics(cfg *schema.OpnSenseDocument) *Statistics {
	stats := &Statistics{
	...
	stats.Summary = StatisticsSummary{
		TotalConfigItems: CalculateTotalConfigItems(ConfigItemCounts{
...
func generateInterfaceStatistics(cfg *schema.OpnSenseDocument, stats *Statistics) {
...
func generateFirewallStatistics(cfg *schema.OpnSenseDocument, stats *Statistics) {
...
func generateGatewayStatistics(cfg *schema.OpnSenseDocument, stats *Statistics) {
	// Gateway statistics
...
func generateDHCPStatistics(cfg *schema.OpnSenseDocument, stats *Statistics) {
	dhcpScopes := 0
...
func generateUserGroupStatistics(cfg *schema.OpnSenseDocument, stats *Statistics) {
...
func generateServiceStatistics(cfg *schema.OpnSenseDocument, stats *Statistics) {
	serviceCount := 0
...
func generateSecurityFeatures(cfg *schema.OpnSenseDocument, stats *Statistics) {
...
func generateAnalysis(cfg *schema.OpnSenseDocument) *Analysis {
	analysis := &Analysis{
...
func analyzeDeadRules(cfg *schema.OpnSenseDocument) []DeadRuleFinding {
	...
	for i, rule := range rules {
		...
		if rule.Type == RuleTypePass && rule.Source.EffectiveAddress() == constants.NetworkAny && rule.Descr == "" {
			findings = append(findings, DeadRuleFinding{
				RuleIndex: i + 1,
				Interface: rule.Interface.String(),
				Description: fmt.Sprintf(
...
func analyzeUnusedInterfaces(cfg *schema.OpnSenseDocument) []UnusedInterfaceFinding {
...
func analyzeSecurityIssues(cfg *schema.OpnSenseDocument) []SecurityFinding {
...
func analyzePerformanceIssues(cfg *schema.OpnSenseDocument) []PerformanceFinding {
...
func analyzeConsistencyIssues(cfg *schema.OpnSenseDocument) []ConsistencyFinding {
...
func generateSecurityAssessment(cfg *schema.OpnSenseDocument) *SecurityAssessment {
...
func generatePerformanceMetrics(cfg *schema.OpnSenseDocument) *PerformanceMetrics {
...
func generateComplianceChecks(cfg *schema.OpnSenseDocument) *ComplianceChecks {
...
func calculateSecurityScore(cfg *schema.OpnSenseDocument, stats *Statistics) int {
	...
	if len(stats.RulesByType) > 0 {
		...
		rules := cfg.FilterRules()
		for _, rule := range rules {
			srcIsAny := rule.Source.EffectiveAddress() == constants.NetworkAny
			dstIsAny := rule.Destination.EffectiveAddress() == constants.NetworkAny
...
func calculateSecurityScoreBase(cfg *schema.OpnSenseDocument, stats *Statistics) int {
