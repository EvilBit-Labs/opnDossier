...
`
	...
	t.Run("custom_template_override", func(t *testing.T) {
		opts := DefaultOptions()
		generator, err := NewMarkdownGeneratorWithTemplates(nil, tempDir, opts)
		require.NoError(t, err, "Failed to create markdown generator with custom templates")

		opts = DefaultOptions().WithFormat(FormatMarkdown).WithComprehensive(false)
...
func TestTemplateOverrideInvalidDirectory(t *testing.T) {
	...
	opts := DefaultOptions()
	generator, err := NewMarkdownGeneratorWithTemplates(nil, nonExistentDir, opts)
...
func TestNewMarkdownGeneratorWithTemplates(t *testing.T) {
	...
	t.Run("without_template_directory", func(t *testing.T) {
		opts := DefaultOptions()
		generator, err := NewMarkdownGeneratorWithTemplates(nil, "", opts)
